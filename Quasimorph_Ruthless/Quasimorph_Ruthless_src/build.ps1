<#
.SYNOPSIS
    Builds the Quasimorph Hardcore Tactical Ruthless mod and assembles the release folder.
...
param(
    ...
    [string]$OutDir = (Join-Path (Split-Path $PSScriptRoot -Parent) "Quasimorph_Ruthless_v0.1"),
...
dotnet build "$Project\mod_src\QuasimorphRuthless\QuasimorphRuthless.csproj" `
...
Copy-Item -Path "$Project\mod_src\QuasimorphRuthless\modmanifest.json" -Destination "$OutDir\mod" -Force
Copy-Item -Path "$stage\QuasimorphRuthless.dll" -Destination "$OutDir\mod" -Force
...
if (Test-Path $checker) {
    python $checker "$stage\QuasimorphRuthless.dll" $GameManaged
...
if ($Install) {
    $target = Join-Path $PresetsDir "QuasimorphRuthless"
