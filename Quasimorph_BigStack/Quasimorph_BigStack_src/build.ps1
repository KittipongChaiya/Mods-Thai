<#
.SYNOPSIS
    Builds the Quasimorph Big Stack mod and assembles the release folder.
...
param(
    ...
    [string]$OutDir = (Join-Path (Split-Path $PSScriptRoot -Parent) "Quasimorph_BigStack_v0.1"),
...
dotnet build "$Project\mod_src\QuasimorphBigStack\QuasimorphBigStack.csproj" `
...
Copy-Item -Path "$Project\mod_src\QuasimorphBigStack\modmanifest.json" -Destination "$OutDir\mod" -Force
Copy-Item -Path "$stage\QuasimorphBigStack.dll" -Destination "$OutDir\mod" -Force
...
if (Test-Path $checker) {
    python $checker "$stage\QuasimorphBigStack.dll" $GameManaged
...
if ($Install) {
    $target = Join-Path $PresetsDir "QuasimorphBigStack"
