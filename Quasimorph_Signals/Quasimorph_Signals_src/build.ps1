<#
.SYNOPSIS
    Builds the Quasimorph Signals mod and assembles the release folder.
...
param(
    ...
    [string]$OutDir = (Join-Path (Split-Path $PSScriptRoot -Parent) "Quasimorph_Signals_v0.1"),
...
dotnet build "$Project\mod_src\QuasimorphSignals\QuasimorphSignals.csproj" `
...
Copy-Item -Path "$Project\mod_src\QuasimorphSignals\modmanifest.json" -Destination "$OutDir\mod" -Force
Copy-Item -Path "$stage\QuasimorphSignals.dll" -Destination "$OutDir\mod" -Force
...
if (Test-Path $checker) {
    python $checker "$stage\QuasimorphSignals.dll" $GameManaged
...
if ($Install) {
    $target = Join-Path $PresetsDir "QuasimorphSignals"
