<#
.SYNOPSIS
    Builds the Quasimorph Silence mod and assembles the release folder.
...
param(
    ...
    [string]$OutDir = (Join-Path (Split-Path $PSScriptRoot -Parent) "Quasimorph_Silence_v0.1"),
...
dotnet build "$Project\mod_src\QuasimorphSilence\QuasimorphSilence.csproj" `
...
Copy-Item -Path "$Project\mod_src\QuasimorphSilence\modmanifest.json" -Destination "$OutDir\mod" -Force
Copy-Item -Path "$stage\QuasimorphSilence.dll" -Destination "$OutDir\mod" -Force
...
if (Test-Path $checker) {
    python $checker "$stage\QuasimorphSilence.dll" $GameManaged
...
if ($Install) {
    $target = Join-Path $PresetsDir "QuasimorphSilence"
