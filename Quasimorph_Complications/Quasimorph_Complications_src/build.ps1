<#
.SYNOPSIS
    Builds the Quasimorph Complications mod and assembles the release folder.
...
param(
    ...
    [string]$OutDir = (Join-Path (Split-Path $PSScriptRoot -Parent) "Quasimorph_Complications_v0.1"),
...
dotnet build "$Project\mod_src\QuasimorphComplications\QuasimorphComplications.csproj" `
...
Copy-Item -Path "$Project\mod_src\QuasimorphComplications\modmanifest.json" -Destination "$OutDir\mod" -Force
Copy-Item -Path "$stage\QuasimorphComplications.dll" -Destination "$OutDir\mod" -Force
...
if (Test-Path $checker) {
    python $checker "$stage\QuasimorphComplications.dll" $GameManaged
...
if ($Install) {
    $target = Join-Path $PresetsDir "QuasimorphComplications"
