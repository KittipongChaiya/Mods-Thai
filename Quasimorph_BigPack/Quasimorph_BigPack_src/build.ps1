<#
.SYNOPSIS
    Builds the Quasimorph Big Pack mod and assembles the release folder.
...
param(
    ...
    [string]$OutDir = (Join-Path (Split-Path $PSScriptRoot -Parent) "Quasimorph_BigPack_v0.1"),
...
dotnet build "$Project\mod_src\QuasimorphBigPack\QuasimorphBigPack.csproj" `
...
Copy-Item -Path "$Project\mod_src\QuasimorphBigPack\modmanifest.json" -Destination "$OutDir\mod" -Force
Copy-Item -Path "$stage\QuasimorphBigPack.dll" -Destination "$OutDir\mod" -Force
...
if (Test-Path $checker) {
    python $checker "$stage\QuasimorphBigPack.dll" $GameManaged
...
if ($Install) {
    $target = Join-Path $PresetsDir "QuasimorphBigPack"
