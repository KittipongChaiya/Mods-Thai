<#
.SYNOPSIS
    Builds the Quasimorph Retinue mod and assembles the release folder.
...
param(
    ...
    [string]$OutDir = (Join-Path (Split-Path $PSScriptRoot -Parent) "Quasimorph_Retinue_v0.1"),
...
dotnet build "$Project\mod_src\QuasimorphRetinue\QuasimorphRetinue.csproj" `
...
Copy-Item -Path "$Project\mod_src\QuasimorphRetinue\modmanifest.json" -Destination "$OutDir\mod" -Force
Copy-Item -Path "$stage\QuasimorphRetinue.dll" -Destination "$OutDir\mod" -Force
...
if (Test-Path $checker) {
    python $checker "$stage\QuasimorphRetinue.dll" $GameManaged
...
if ($Install) {
    $target = Join-Path $PresetsDir "QuasimorphRetinue"
